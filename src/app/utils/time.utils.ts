...
export const generateFullDateStringV2 = ({ day, month, year }: { day: number; month: number; year: number }): string => {
      const result = `${fillTwoCharDate(day)}-${fillTwoCharDate(month)}-${year}`;
      return result;
};
...
export const getNameWeekDate = ({ date, locale = "en-EN" }: { date: string; locale?: string }): UI.Calender.DateNameInWeek => {
