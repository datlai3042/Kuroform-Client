...
class FormService {
      ...
      static async getFormPagination({ page, limit }: { page: number; limit: number }) {
            return Http.get<ResponseApi<{ forms: FormCore.Form[]; total_page: number }>>(`/v1/api/form/get-form-pagination?page=${page}&limit=${limit}`);
      }

      static async getAllFormState() {
            return Http.get<ResponseApi<{ formDelete: number; formPublic: number; formPrivate: number }>>(`/v1/api/form/get-all-form-state`);
      }
      ...
      static async addSubTitleItem({ type, form_id }: { type: FormCore.FormTitleSub.FormTitleBase["type"]; form_id: string }) {
      ...
      static async deleteTitleSubItem({ title_sub_id, form_id }: { title_sub_id: string; form_id: string }) {
            return Http.delete<ResponseApi<{ form: FormCore.Form }>>(`/v1/api/form/delete-sub-title-item?form_id=${form_id}&title_sub_id=${title_sub_id}`);
      }
      ...
      static async setModeImageForm({ form_id, mode }: { form_id: string; mode: FormCore.FormTitle["form_title_mode_image"] }) {
      ...
      static async deleteFormId({ form_id }: { form_id: string }) {
            return Http.delete<ResponseApi<{ form: FormCore.Form; formPrivate: number; formPublic: number; formDelete: number }>>(
                  `/v1/api/form/delete-form-id?form_id=${form_id}`,
            );
      }

      static async getFormGuess({ form_id, options }: { form_id: string; options?: RequestInit }) {
            return Http.get<ResponseApi<{ form: FormCore.Form; form_answer_id: string }>>(`/v1/api/form/get-form-guess?form_id=${form_id}`, options);
      }
      ...
      static async getInfoFormNotification({ form_id, notification_id }: { form_id: string; notification_id: string }) {
            return Http.get<ResponseApi<{ form: Pick<FormCore.Form, "form_avatar" | "form_title" | "form_setting_default"> }>>(
