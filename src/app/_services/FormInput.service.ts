...
class FormInputService {
      ...
      static async removeInputItem({ form_id, input_id }: { form_id: string; input_id: string }) {
            return Http.post<ResponseApi<{ form: FormCore.Form }>>("/v1/api/form/delete-input-item", {
                  form_id,
                  input_id,
            });
      }

      static async setTitleInput({ input_title_value, input_id, form }: { input_title_value: string; input_id: string; form: FormCore.Form }) {
      ...
      static async changeTypeInput({ form, inputItem, type }: { form: FormCore.Form; inputItem: InputCore.InputForm; type: InputCore.InputForm["type"] }) {
      ...
      static async updatePostionOption({
            ...
            form: FormCore.Form;
            inputItem: InputCore.InputOption.InputTypeOption | InputCore.InputOptionMultiple.InputTypeOptionMultiple;
            coreOption: InputCore.InputOption.InputTypeOption["core"]["options"] | InputCore.InputOptionMultiple.InputTypeOptionMultiple["core"]["options"];
      }) {
