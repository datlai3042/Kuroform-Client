import { onUpdateToastGlobal, removeOneToast } from "@/app/_lib/redux/toast.slice";
...
const ToastWrapper = (props: TProps) => {
      ...
      return (
            <div
                  style={{ top: indexItem === 0 ? indexItem : indexItem * 110 }}
                  className={`${
                        addAnimation ? "animate-hiddenToast absolute" : ""
                  } shadow-2xl transition-all duration-500 absolute  w-full min-h-[8rem] h-max rounded-lg `}
            >
                  {children}
            </div>
      );
