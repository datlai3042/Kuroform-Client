...
const Logo = () => {
      ...
      return (
            <Link href={"/"}>
                  ...
                  {!user?.user_avatar_current && (
                        <div className="w-[10rem] h-[10rem] bg-color-main text-[#fff] flex justify-center items-center text-[1.6rem] uppercase object-cover rounded-full">{user?.user_first_name.slice(0, 1) || user?.user_email?.slice(0, 1)}</div>
                  )}
            </Link>
      );
};
