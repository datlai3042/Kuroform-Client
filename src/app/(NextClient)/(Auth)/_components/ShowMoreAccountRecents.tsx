...
const ShowMoreAccountRecents = () => {
  ...
  return (
    <>
      ...
      <ModalShowMoreAccountRecents
        openModel={showMoreAccount}
        setOpenModel={setShowMoreAccount}
      />
    </>
  );
};
...
const ShowMoreAccountContent = (props: TShowMoreAccountContentProps) => {
  const { onClose } = props;
  const [userRecents, setUserRecents] = useState<TUserRecent[]>(
    JSON.parse(localStorage.getItem("userRecents")!) || []
  );
  ...
  return (
    <>
      <div
        className="p-[3rem] max-w-[60vw] max-h-[80vh] flex flex-col gap-[1rem]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between">
          <div className="flex items-center gap-[1rem]  gradient-app-name text-primary-color">
            <UserPlus />
            <span className="text-[2rem] text-primary-color font-semibold">
              Các Account đã login
            </span>
          </div>
    ...
    <div className="flex gap-[1.4rem] hover:bg-gray-200 justify-between border-[.1rem] border-border-page-color p-[.6rem_1rem] rounded-[.8rem]">
      <div className="flex items-center">
        <Image
          ...
          src={user?.avatar}
          alt="user_recents"
          className="w-[9rem] h-[9rem] object-cover rounded-lg"
        />
      </div>
