...
const InputPhoneIntroduce = (props: TProps) => {
      ...
      return (
            <DivNative className="w-full h-full flex flex-col  sm:pt-[4rem] ">
                  <InputIntroduceHeader title={inputIntroducePhone.title} description={inputIntroducePhone.description} action={handleChooseInputType} />
                  <DivNative className="h-[50%] flex flex-col gap-[1.8rem] p-[2rem] ">
                        ...
                        <DivNative className="flex flex-col gap-[1rem]">
                              <SpanNative textContent="Số điện thoại" className="text-[1.6rem] font-semibold" />
                              <DivNative className={` relative min-h-[5rem] h-max flex items-center gap-[.5rem] `}>
                                    <input
                                          value={phone}
                                          type="number"
                                          className="w-full h-full p-[1rem] rounded-lg text-[1.6rem]   border-[.1rem] border-[var(--border-color-input)] bg-color-section-theme text-text-theme  outline-none "
                                          placeholder="Nhập số điện thoại của bạn"
                                          onChange={(e) => setPhone(+e.target.value)}
                                    />
                              </DivNative>
                              <span className="text-[1.4rem]">
                                    Số điện thoại bạn nhập {phone > 0 && phone.toString().match(regexPhoneVietNam) ? "hợp lệ" : "không hợp lệ"}
                              </span>
                        </DivNative>
                  </DivNative>
            </DivNative>
