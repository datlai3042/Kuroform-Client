	...
	return (
		<>
			{data_in_month.length > 0 && (
				<div className="flex flex-wrap xl:justify-end gap-[.4rem] mt-[1rem] w-full h-[18rem] xl:h-[21rem] rounded-xl   text-[1.3rem]">
						...
						if (data.match) {
							return (
								<button
									...
									className={`bg-color-main text-[#fff] w-[3rem] h-[3rem] flex items-center justify-center rounded-md `}
								>
									{data.date_createdAt}
								</button>
							);
							...
							return (
								<button
									...
									className={`bg-slate-700 text-[#fff] w-[3rem] h-[3rem] flex items-center justify-center rounded-md `}
								>
									{data.date_createdAt}
								</button>
							);
					...
					<div className="text-text-theme w-full flex justify-end items-center ">
						Bạn đã đang{" "}
						<span className="text-color-main mx-[.4rem] font-extrabold">{form_create_in_month}</span> trong
						tháng <span className="text-color-main mx-[.4rem] font-extrabold">{month}</span>
					</div>
				</div>
			)}
			...
			{data_in_month.length === 0 && (
				...
				<div className="flex flex-wrap xl:justify-end gap-[.4rem] w-full h-[18rem] xl:h-[21rem] rounded-xl   text-[1.3rem]">
					...
					<div className="text-text-theme w-full flex justify-end items-center">Không có dữ liệu</div>
				</div>
			)}

			{/* {data_form_final.length === 0 && (
				<button
					key={i}
					title="Không có dữ liệu"
					className={`bg-slate-700 text-[#fff] w-[3rem] h-[3rem] flex items-center justify-center rounded-md `}
			...
			{openModelDetail && (
				<DashboardFilterDetails
					forms={selectData?.forms!}
					date_full={selectData?.date_full!}
