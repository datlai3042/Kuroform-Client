...
const DashboardSearchResult = (props: TProps) => {
	...
	return (
		<div className="min-w-[30rem] w-full bg-[#fff] min-h-full h-full max-h-max">
			{formSearchResult?.data?.metadata?.forms?.map((form) => (
				<DashboardFormSearchItem form={form} key={form._id} />
			))}
			...
			{!search && (
				<div className="min-w-full min-h-[4rem] flex items-center px-[1rem]">
					Hãy nhập tên Form bạn muốn tìm kiếm
				</div>
			)}
		</div>
	);
