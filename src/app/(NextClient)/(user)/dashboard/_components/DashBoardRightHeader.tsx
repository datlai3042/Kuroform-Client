		...
		>
			<div className="h-full w-full flex items-center justify-between">
				<div className="flex gap-[1rem]">
					{!openSidebar && (
						<div className="flex items-center gap-[1rem] ">
							<ButtonIcon
								Icon={<ChevronsRight className="text-text-theme" />}
								onClick={() => setOpenSidebar(true)}
							/>
						</div>
					)}
					<p className="hidden xl:block text-text-theme text-[2rem] font-semibold">
						{generateContentToUrl(pathName)}
					</p>
				</div>
				...
				<div className="flex justify-end gap-[1rem] xl:gap-[20px]">
					<Link
						href={"/settings"}
						className="p-[.2rem_.8rem] flex items-center gap-[.8rem] text-text-theme rounded-md"
						title="Cài đặt"
					>
