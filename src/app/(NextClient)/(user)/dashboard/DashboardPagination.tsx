...
const DashboardPagination = (props: TProps) => {
	...
	const onStylePageCurrent = (_index: number) =>
		_index === page
			...
			: " bg-color-section-theme text-text-theme hover:text-[#fff] hover:bg-blue-600";
	...
	return (
		<div className="text-text-theme flex flex-col xl:flex-row items-center justify-center gap-[2rem] xl:gap-0 h-[4rem] w-full">
				...
				{pageRender.map((pageItem) => (
					<button
						...
						className={`${onStylePageCurrent(
							pageItem
						)} w-[3rem] aspect-square rounded-full flex justify-center items-center`}
					>
						<span>{pageItem}</span>
					</button>
				))}
				<div className="absolute right-0 min-w-[12rem] flex  gap-[1rem]">
					Tổng trang:{" "}
